 <?php session_start();
	...
	$upit_pisma="SELECT * FROM pisma p JOIN korisnici k ON p.id_posiljaoca=k.id_korisnika WHERE p.id_primaoca='".$korisnik."' AND p.status='1' ORDER BY p.id_pisma";
	$rezultat_pisma=mysql_query($upit_pisma,$konekcija);
	if($rezultat_pisma){
		$broj_pisama=mysql_num_rows($rezultat_pisma);
	}
	
	$upit_pisma="SELECT * FROM pisma p JOIN korisnici k ON p.id_posiljaoca=k.id_korisnika WHERE p.id_primaoca='".$korisnik."' AND p.status='1' ORDER BY p.id_pisma DESC LIMIT $broj_po_strani OFFSET $s";
	$rezultat_pisma=mysql_query($upit_pisma,$konekcija);
	if($rezultat_pisma){
	...
	if($rezultat_pisma){
		$broj=mysql_num_rows($rezultat_pisma);
		echo "<div class='admin_title'>Nepročitane poruke</div>";
		if($broj>0){
			while($pismo=mysql_fetch_array($rezultat_pisma)){
				...
				echo "<div class='message_center'><a href='show_new_message.php?message=".$pismo['id_pisma']."&user=".$pismo['id_primaoca']."'>";
			...
			if($broj_pisama<=$broj_po_strani){
				echo "<div id='funkcija'><input type='hidden' id='izvrsi' value='show_new_messages'/></div>";
				echo "<div><input type='hidden' id='korisnik' value='".$korisnik."'/></div>";
			}else if($left<0){
				echo "<input type='button' id='napred' value='' onclick='next(".'"show_new_messages"'.");' class='next'/>";
				...
				echo "<input type='button' id='nazad' value='' onClick='back(".'"show_new_messages"'.");' class='back'/>";
				echo "<div id='stranicenje'><input type='hidden' id='s' value='".$s."'/></div>";
				echo "<div id='funkcija'><input type='hidden' id='izvrsi' value='show_new_messages'/></div>";
				echo "<div><input type='hidden' id='korisnik' value='".$korisnik."'/></div>";
